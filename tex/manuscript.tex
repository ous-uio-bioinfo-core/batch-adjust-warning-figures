...
In an experiment described in \citet{Towfic2014}, the effect of glatiramer acetate (a medicine for multiple sclerosis) was compared to the effect of a generic drug. Cells were treated with
glatiramer acetate (34 samples), the generic drug (11 samples) or 14 different types of other treatments(64 samples), and mRNA was measured using microarrays. A batch effect correlating to the chip (Illumina
WG-6\_V2, six samples per chip, 17 chips in total) was observed and adjusted for with
ComBat, whereafter the data were tested for differentially expressed
genes, yielding hundreds of differentially expressed probes (Table S5, \citealp{Towfic2014}) using a $\text{FDR}<0.05 threshold.
Unfortunately, the batch--treatment design was unbalanced, with several
batches having only one of the main treatments of interest. When we
re-analyzed the data without the use of ComBat, but instead blocked for
batch effect in \texttt{limma}, only 9 genes were found ($\text{FDR}<0.05$)
. The sanity check  outlined above with random numbers
was also carried out. The distribution of p-values for different settings
are shown in Figure~\ref{fig:comparison}a. Our conclusion is that most of
the genes found as differentially expressed in the analysis described in
\citet{Towfic2014} are false positives. This example represents somewhat
of a ``worst case'' scenario for applying ComBat, since it both has a
very unbalanced batch--group design and an a priori assumption of no
difference. Our re-analysis, although based on the "methods" section of
Towfic et al, deviates in important ways from the analysis that was
actually performed in the paper (personal communication, Ben Zeskind).
More detailed information on this aspect can be found in the extended github repostiory
...
\begin{center}$
\begin{array}{cc}
\includegraphics[width=4cm]{Fig/leekrandomdatapvalues.pdf} &
\includegraphics[width=4cm]{Fig/leekqqplot.png}\includegraphics[width=4cm]{Fig/samplesizescaling_ComBat.pdf}
\end{array}$
\end{center}
\caption
{
...
e) Simulated expression of one gene in 20 samples from two groups (circles and triangles). The t-test indicates no significant group differences(p=0.82).
f) The values from (e) are divided into 3 batches in an unbalanced way, with only batch 2 containing samples from both groups. Batch effects were added, +1 for batch 1 and -1 for batch 3. The t-test now results in p<0.00. This plot illustrates the problem with disregarding batch effects.
g) The values in (f) were batch effect adjusted while retaining the group difference (using removeBatchEffects), thus creating a ``batch effet free'' data set. Notice that all the values in batch 1 (which also belong to the circle group) are adjusted to make the group mean coincide with the mean of the two circle-samples in batch 2. The triangle-samples in batch 3 is adjusted similarly to match the mean of the two triangle-sampels in batch 2.The t-test now results in a significant group difference (p=0.01) and a fold change equal to a fold change calculated from the batch 2 samples only. Thus, the fold change is in effect estimated from only 2+2 samples, but the t-test assumes 10+10 samples with the low p-value as a result.
