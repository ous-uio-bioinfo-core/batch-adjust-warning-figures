...
\begin{abstract}
{Removal of, or adjustment for, batch effects or centre differences is generally required when such effects are present in data. In particular, when preparing microarray gene expression data from multiple cohorts, array platforms, or batches for later analyses, batch effects can have confounding effects. Many methods and tools exist for this purpose. One method, ComBat, which is part of the R package sva, is particularly popular due to its ability to remove batch differences even when batches are small and heterogeneous. It also has the option of preserving the difference between study groups, estimated from a two-way ANOVA model, to avoid conflating batch effects and group differences during batch adjustments. This is a natural approach when study groups are not balanced between batches as batch adjustments may otherwise bias, usually deflate, group differences. Unfortunately, this frequently used and recommended approach may systematically induce incorrect group differences in downstream analyses when groups are distributed between the batches in an unbalanced manner. \COMMENT{The scientific community seems to be largely unaware of how this approach may lead to false discoveries.}{Har moderert påstanden noe, jfr. reviewer~2, pt.~6.} We would expect any method for removing batch effects while retaining group differences to run into the same problem.
}
\end{abstract}
...
In conclusion, when study groups are unevenly distributed across batches, batch effects can have confounding effects if not properly adjusted for. This is difficult to do through a two-step procedure as batch adjustment will not be able to produce a ``batch effect free'' data set, as has also been observed in \citet{Buhule2014}. Estimating batch and group effects simultaneously through two-way ANOVA tries to overcome this, and does succeed at removing systematic biases in the group differences induced by the batch effects, but at the cost of making the observations interdependent to an extent that may invalidate subsequent analyses. Proper statistical analysis of unbalanced designs require models that handle batch effects as part of the analysis. However, the best way to handle this will be to ensure a balanced study design from the start, both to avoid data analysis problems and the loss of statistical power the ensues when batch and group effects need to be disentangled.
...
\NB{Einars tekst:} It should be noted that the original analyses by \citet{Towfic2014} differed from our re-analysis in one critical manner which was clarified upon contacting the authors. Their analyses used a different preprocessing of the microarray data (GEO: GSE61901) in which two technical replicates of each biological sample was included, corresponding to the two different slots of the bead-microarray. They refer to \citet{Bakshi2013} for analyses where the two slots have been combined to provide one set of expression values per sample.

\NB{Vegards tekst:} Our conclusion is that most of the genes found as differentially expressed in the analysis described in \citet{Towfic2014} are false positives. This example represents somewhat of a ``worst case'' scenario for applying ComBat, since it both has a very unbalanced batch--group design and an a priori assumption of no difference. Our re-analysis, although based on the "methods" section of Towfic et al, deviates in important ways from the analysis that was actually performed in the paper (personal communication, Ben Zeskind). More detailed information on this aspect can be found in the extended github repostiory.
